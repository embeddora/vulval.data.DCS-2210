/* $Id: upnpc.c,v 1.6 2010/08/12 10:10:02 jiahung Exp $ */
/* Project : miniupnp
                         ...
                         struct IGDdatas * data)
{
	...
	if(r != UPNPCOMMAND_SUCCESS){
		...
		pSysInfo->status.upnpportforewarding = 0;
	}else
		pSysInfo->status.upnpportforewarding = 1;
	...
	pSysInfo->ipcam[UPNPFOWD_STATUS].config.value = pSysInfo->status.upnpportforewarding;
                               ...
                               const char * proto)
{
	...
	//pSysInfo->config.lan_config.net.upnpfowd_status = r;
	//pSysInfo->ipcam[UPNPFOWD_STATUS].config.value= r;
	
	//printf("UPNP_AddPortMapping = %d\n", r);
...
//printf("UPNP_GetSpecificPortMappingEntry = %d\n", r);
	...
	if(getexport_error == 0 && r == UPNPCOMMAND_SUCCESS)
		pSysInfo->status.upnpportforewarding = 1;
	else
		pSysInfo->status.upnpportforewarding = 0;
	
	pSysInfo->ipcam[UPNPFOWD_STATUS].config.value= pSysInfo->status.upnpportforewarding;
	printf("pSysInfo->status.upnpportforewarding = %d\n", pSysInfo->status.upnpportforewarding);
}
...
int main(int argc, char ** argv)
{
	...
	//pSysInfo->config.lan_config.net.upnpfowd_status = -1;
    pSysInfo->ipcam[UPNPFOWD_STATUS].config.value = -1;
	pSysInfo->status.upnpportforewarding = 0;
