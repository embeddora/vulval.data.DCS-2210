...
ushare_new (void)
{
  ...
  ut->name = NULL;
  ut->interface = strdup (pSysInfo->ifether);
  ut->model_name = strdup (pSysInfo->oem_data.model_name);
  ut->contentlist = NULL;
  ...
  net_get_hwaddr(pSysInfo->ifether, mac);
  sprintf(ut->mac_str, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
...
init_upnp (struct ushare_t *ut)
{
  ...
  int res;
  size_t len;
  //unsigned short http_port = pSysInfo->config.lan_config.net.http_port;
  unsigned short http_port = pSysInfo->ipcam[HTTP_PORT].config.u16;
  char ip_port[32];
...
#endif /* HAVE_DLNA */ 
  if (http_port != 80)
    sprintf(ip_port, "%s:%d", ut->ip, http_port);
  else
    strcpy(ip_port, ut->ip);

#ifdef CONFIG_BRAND_DLINK
  model_serial = ut->model_name;
#else
  model_serial = ut->mac_str;
#endif

  len = strlen (UPNP_DESCRIPTION) + strlen (ut->name) + strlen (pSysInfo->oem_data.upnp_manufacturer)
  		+ strlen(pSysInfo->oem_data.upnp_manufacturer_url)*2 + strlen(pSysInfo->oem_data.upnp_description)
		+ strlen (ut->model_name) + strlen (model_serial) + strlen (ut->udn) + strlen(ip_port) + 1;
  description = (char *) malloc (len * sizeof (char));
  memset (description, 0, len);
  sprintf (description, UPNP_DESCRIPTION, ut->name, pSysInfo->oem_data.upnp_manufacturer, 
  				pSysInfo->oem_data.upnp_manufacturer_url, pSysInfo->oem_data.upnp_description,
		  		ut->model_name, model_serial, pSysInfo->oem_data.upnp_manufacturer_url, ut->udn, ip_port);
#ifdef HAVE_DLNA
  }
#endif /* HAVE_DLNA */
...
create_udn (char *interface)
{
  ...
  snprintf (buf, 64, "%s-%02x%02x%02x%02x%02x%02x", pSysInfo->oem_data.model_name,
...
static char *get_friendly_name(char *ipaddr)
{
  char *val;

  val = (char *) malloc (MAX_LANCAM_TITLE_LEN + 20);
  //sprintf (val, "%s (%s)", pSysInfo->config.lan_config.title, ipaddr);
  sprintf (val, "%s (%s)", conf_string_read(TITLE), ipaddr);
...
restart_upnp (struct ushare_t *ut)
{
  ...
  if (ut->name)
    free (ut->name);
  ut->name = get_friendly_name (ut->ip);
  if (!ut->name)
    return -1;
...
{
	FILE *fp_pid;
	if( access(UPNP_AV_PID , F_OK) == 0 ){
		DBG("UPnP server running.\n");
		return 0;
	}
	if ((fp_pid = fopen(UPNP_AV_PID, "w")) == NULL) {
		DBGERR("open file " UPNP_AV_PID "error");
		return EXIT_FAILURE;
	};
	fprintf(fp_pid, "%d\n", getpid());
	fclose(fp_pid);
  ...
  signal (SIGUSR1, refresh_file);
  signal (SIGUSR2, refresh_descript);
