...
static int rtsp_o_http_read_body(request * req);
        ...
        if (req->status == ONE_LF) {
            *req->header_end = '\0';
            ...
            if (retval && req->method == M_POST) {
                ...
                if ((req->http_uri) && (req->http_uri->uri_flag & URI_FLAG_RTSP_O_HTTP))
                {
                    if ( retval == 2 )
                    {
                        memcpy( req->client_stream,
                                req->client_stream + req->parse_pos,
