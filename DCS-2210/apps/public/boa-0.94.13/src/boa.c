...
#include <sysinfo.h>
#include <utility.h>
#include <sys/types.h>  //add by mikewang,used for UPnP control
#include <signal.h>  //add by mikewang,used for UPnP control
#endif
...
#ifdef DAVINCI_IPCAM
SysInfo *sys_info;
char http_stream[MAX_PROFILE_NUM][MAX_HTTPSTREAMNAME_LEN+2];
char rtsp_stream[MAX_PROFILE_NUM][MAX_HTTPSTREAMNAME_LEN+2];
#endif  // DAVINCI_IPCAM
...
int main(int argc, char **argv)
{
	...
	sys_info = (SysInfo *)sysinfo_init(sizeof(SysInfo), 1);
	//sys_info = (SysInfo *)sysinfo_init(sizeof(SysInfo));
	if (sys_info == NULL) {
		dbg("sysmem_init() failed\n");
		exit(1);
	}

	int idx;
#if 0//def SUPPORT_PROFILE_NUMBER //modify by jiahung, 2011.05.04
	for (idx=0; idx<sys_info->ipcam[CONFIG_MAX_WEB_PROFILE_NUM].config.value; idx++) {
		sprintf(http_stream[idx], "/%s", conf_string_read(STREAM0_HTTPSTREAMNAME+2*idx));
		sprintf(rtsp_stream[idx], "/%s", conf_string_read(STREAM0_RTSPSTREAMNAME+2*idx));
		//sprintf(http_stream[idx], "/%s", sys_info->config.lan_config.stream[idx].httpstreamname);
		dbg("http stream name %d = %s", idx, http_stream[idx]);
	}
#else
	...
	sys_info->pid.boa = getpid();
	// decrease the process priority
	//setpriority(PRIO_PROCESS, 0, 1);
#ifdef SERVER_SSL
	do_ssl = sys_info->ipcam[HTTPS_ENABLE].config.u8;
	//do_ssl = sys_info->config.https.https_enable;
#endif

	setenv("TZ", sys_info->tzenv, 1);
#if 1
	if (sys_info->ipcam[RS485_ENABLE].config.u8) {
	//if (sys_info->config.rs485.enable) {
		if(rs485_init(sys_info) == -1){
			DBG("rs485_init failed \n");
			exit(1);
		}
	}
#endif // RS485_ENABLE
    ...
    read_config_files();
#ifdef DAVINCI_IPCAM
	int port = sys_info->ipcam[HTTP_PORT].config.u16;
	//int port = sys_info->config.lan_config.net.http_port;
	if (port == 80 || port >= MIN_HTTP_PORT)
		server_port = port;
	dbg("server_port=%d\n", server_port);
#ifdef SERVER_SSL
	port = sys_info->ipcam[HTTPS_PORT].config.u16;
	//port = sys_info->config.https.port;
	if (port == 443 || port >= MIN_HTTPS_PORT)
		ssl_server_port = port;
	dbg("SSL server port: %d", port);
#endif // SERVER_SSL
