...
int init_cgi(request * req)
{
    ...
    if ((req->is_cgi == CGI || 1) && !(req->http_uri && (req->http_uri->uri_flag & URI_FLAG_DIRECT_WRITE))) {
        use_pipes = 1;
        if (pipe(pipes) == -1) {
            log_error_time();
            perror("pipe");
...
#if 0 // nedwu
    if ( req->http_uri && ! strcmp( req->http_uri->name, "/live1.sdp" ) && req->method == M_POST )
    {
      if ( req->client_stream_pos - req->parse_pos )
      {
      	*( req->client_stream + req->client_stream_pos ) = 0;
        add_cgi_env( req, "HAVEDATA", req->client_stream + req->parse_pos, 0 );
      }
    }
#endif
    child_pid = fork();
