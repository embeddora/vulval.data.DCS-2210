...
#include "boa.h"
#include "rs485.h"				/* add by Alex, 2009.01.09	*/
...
//    req_timeout.tv_usec = 0l;   /* reset timeout */
    ...
    while (1) {
        ...
        while ( req = rtsp_o_http_get_return_req() )
...
#else
        sys_info->wdt.boa = 1;
        req_timeout.tv_sec = (request_ready ? 0 : 1);
        req_timeout.tv_usec = 0l;   /* reset timeout */

        if (select(max_fd + 1, &block_read_fdset,
                   &block_write_fdset, NULL, &req_timeout) == -1) {
            /* what is the appropriate thing to do here on EBADF */
            if (errno == EINTR)
                continue;   /* while(1) */
            else if (errno != EBADF) {
...
#ifdef DBG_BLOCKED_SOCKET
  {
	static int old_blocked_req = -1;
	int blocked_req = 0;
	char buffer[50];
	for(current=request_block; current; current=next)
		blocked_req++;
    if (old_blocked_req != blocked_req) {
    	sprintf(buffer, "%d request blocked ", blocked_req);
    	osd_print(5, PST_BLOCKED_SOCKET, buffer);
    	old_blocked_req = blocked_req;
	}
  }
#endif
 }
