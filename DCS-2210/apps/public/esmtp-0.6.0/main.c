...
int main (int argc, char **argv)
{
	...
	while ((c = getopt (argc, argv, "A:B:P:U:Z:z:b:C:cd:e:E:F:f:Gh:IiL:M:mN:nO:o:p:q:R:r:sTtV:vXY:")) != EOF)
		switch (c)
		{
			case 'A':
			...
			case 'Z':	//mikewang
				...
				break;
			case 'z':	//mikewang
				server_ip = malloc(strlen(optarg)+1);
			...
			case 'Y':
				test_mode = 1;
				fprintf(stderr, "ENABLE TEST MODE ON \n");
				break;
	...
	if( server_port == NULL ){	//mikewang
		printf("serevr port not set.\n");
		return -1;
	}
	if( server_ip == NULL ){	//mikewang
		printf("server_ip port not set.\n");
		return -1;
	}
	
	if(atexit(smtp_quit)) fprintf(stderr, "\n=====esmtp atexit error=====\n");
