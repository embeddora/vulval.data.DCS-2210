...
void smtp_send(message_t *msg, identity_t *identity)
{
...
#endif
	if(test_mode) {
		//if( (sinfo = (SysInfo*)sysinfo_init(sizeof(SysInfo))) == NULL )
		if( (sinfo = (SysInfo*)sysinfo_init(sizeof(SysInfo), 1)) == NULL )
			goto failure;
	}

#if defined( SUPPORT_QOS_SCHEDULE )
	if ( seteuid( 9 ) )	/* set Effective UID to 'mail' */
		perror( "seteuid" );
#endif
	 ...
	 */
	if(identity->host)
		identity->host = (char*)realloc(identity->host, strlen(server_ip) + strlen(server_port) + 2);
	else
		identity->host = (char*)malloc(strlen(server_ip) + strlen(server_port) + 2);
	sprintf( identity->host ,"%s:%s",server_ip,server_port);	//mikewang
	...
	if(username) {
		identity->user = (char*)realloc(identity->user, strlen(username) + 1);
		memset(identity->user, 0, strlen(username) + 1);
		strcpy( identity->user , username );	//mikewang
	}
	
	if(password) {
		identity->pass = (char*)realloc(identity->pass, strlen(password) + 1);
		memset(identity->pass, 0, strlen(password) + 1);
		strcpy( identity->pass , password );	//mikewang
	}
	...
	if (!smtp_start_session (session))
	{
		...
		if(test_mode)
			sinfo->testserver = TEST_SERVER_FAIL;
	...
	if(test_mode)
		sinfo->testserver = TEST_SERVER_SUCESS;
